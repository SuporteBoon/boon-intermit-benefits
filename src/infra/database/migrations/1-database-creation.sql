...
CREATE TABLE tb_usuario (
 ...
 perfil_id BIGINT REFERENCES tb_perfil(id),
 papeis TEXT,
...
CREATE TABLE tb_estipulante_faixa_contribuicao (
 id BIGSERIAL PRIMARY KEY, estipulante_id BIGINT REFERENCES tb_estipulante(id),
 faixa_inicial NUMERIC(10,2), faixa_final NUMERIC(10,2),
 percentual NUMERIC(5,2), descricao VARCHAR(200), ativo BOOLEAN
);

CREATE TABLE tb_estipulante_contato (
 id BIGSERIAL PRIMARY KEY, estipulante_id BIGINT REFERENCES tb_estipulante(id),
 tipo_contato VARCHAR(50), nome VARCHAR(200), email VARCHAR(200),
...
CREATE TABLE tb_estipulante_endereco (
 id BIGSERIAL PRIMARY KEY, estipulante_id BIGINT REFERENCES tb_estipulante(id),
 tipo_endereco VARCHAR(50), cep VARCHAR(9), logradouro VARCHAR(300),
...
CREATE TABLE tb_estipulante_documento (
 id BIGSERIAL PRIMARY KEY, estipulante_id BIGINT REFERENCES tb_estipulante(id),
 tipo_documento VARCHAR(100), nome_arquivo VARCHAR(300),
 caminho VARCHAR(500), data_upload TIMESTAMPTZ,
 usuario_upload_id BIGINT REFERENCES tb_usuario(id)
);

CREATE TABLE tb_subestipulante_dados_bancarios (
 id BIGSERIAL PRIMARY KEY, subestipulante_id BIGINT REFERENCES tb_subestipulante(id),
 banco VARCHAR(100), agencia VARCHAR(10), conta VARCHAR(20),
 tipo_conta VARCHAR(30), titular VARCHAR(200),
 cpf_cnpj_titular VARCHAR(18), principal BOOLEAN
);
...
CREATE TABLE tb_subestipulante_cargo (
 id BIGSERIAL PRIMARY KEY, subestipulante_id BIGINT REFERENCES tb_subestipulante(id),
 codigo VARCHAR(50), nome VARCHAR(200), job_level VARCHAR(100),
 ativo BOOLEAN
);
...
CREATE TABLE tb_contrato (
 ...
 estipulante_id BIGINT REFERENCES tb_estipulante(id),
 produto VARCHAR(200),
 operadora_fornecedor_id BIGINT REFERENCES tb_operadora(id),
 sistema_digitacao VARCHAR(100),
 parceiros_adm_beneficios VARCHAR(200),
 contrato_conjugado BOOLEAN,
 codigo_contrato_apolice VARCHAR(100),
 ...
 data_vigencia_inicio DATE, data_aniversario_contrato DATE,
 ...
 descricao TEXT, ativo BOOLEAN,
 data_cadastro TIMESTAMPTZ, data_atualizacao TIMESTAMPTZ,
 usuario_cadastro_id BIGINT REFERENCES tb_usuario(id)
);

CREATE TABLE tb_contrato_produto (
 id BIGSERIAL PRIMARY KEY, contrato_id BIGINT REFERENCES tb_contrato(id),
...
CREATE TABLE tb_contrato_subestipulante (
 ...
 ativa BOOLEAN, data_inicio DATE, codigo_na_operadora VARCHAR(100),
...
CREATE TABLE tb_procedimento (
 id BIGSERIAL PRIMARY KEY, nome VARCHAR(300),
 categoria VARCHAR(100), ativo BOOLEAN
);

CREATE TABLE tb_plano (
 id BIGSERIAL PRIMARY KEY, contrato_id BIGINT REFERENCES tb_contrato(id),
 operadora_id BIGINT REFERENCES tb_operadora(id),
 produto VARCHAR(200), codigo VARCHAR(50),
 cobertura VARCHAR(200), acomodacao VARCHAR(30),
 abrangencia VARCHAR(30), ativo BOOLEAN,
 data_cadastro TIMESTAMPTZ
);

CREATE TABLE tb_plano_procedimento (
 id BIGSERIAL PRIMARY KEY, plano_id BIGINT REFERENCES tb_plano(id),
 procedimento_id BIGINT REFERENCES tb_procedimento(id),
 ativo BOOLEAN
);
...
CREATE TABLE tb_faturamento (
 ...
 estipulante_id BIGINT REFERENCES tb_estipulante(id),
 operadora_fornecedor_id BIGINT REFERENCES tb_operadora(id),
 contrato_id BIGINT REFERENCES tb_contrato(id),
 subestipulante_id BIGINT REFERENCES tb_subestipulante(id),
 estrutura_empresarial VARCHAR(100), ano_mes_competencia VARCHAR(7),
 ...
 status VARCHAR(30), data_cadastro TIMESTAMPTZ,
 data_atualizacao TIMESTAMPTZ, usuario_cadastro_id BIGINT REFERENCES tb_usuario(id)
);
...
CREATE TABLE tb_cobertura_beneficiario (
 ...
 contrato_origem_id BIGINT REFERENCES tb_contrato(id),
 data_inicio_cobertura DATE,
 data_fim_cobertura DATE,
